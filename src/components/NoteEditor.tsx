...
export function NoteEditor({ note, naming, canDelete, onSave, onDelete, onClose }: Props) {
  ...
  return <section className="note-editor" aria-label="Editar nota">
    <div><h2>Editar {midiToDisplayName(midi, naming)}</h2><button className="close" aria-label="Fechar editor" onClick={onClose}>×</button></div>
    <label>Nota MIDI<input type="number" min="0" max="127" step="1" value={midi} onChange={(event) => setMidi(Number(event.target.value))} /><small>{midiToDisplayName(midi, naming)}</small></label>
