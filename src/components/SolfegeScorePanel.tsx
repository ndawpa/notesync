...
export function SolfegeScorePanel({ score }: { score: SolfegeScore }) {
  ...
  return <section className="score-panel solfege-score">
    ...
    {errors.length > 0 && <div className="solfege-errors"><h3>Pontos a revisar</h3><ul>{errors.map((item) => <li key={item.noteId}>Nota {item.noteId}: esperado <strong>{item.expected}</strong>, {item.recognized ? <>reconhecido <strong>{item.recognized}</strong>{item.timingErrorMs !== undefined && <> · entrada {Math.abs(item.timingErrorMs)} ms {item.timingErrorMs < 0 ? 'adiantada' : 'atrasada'}</>}</> : 'não reconhecida'}</li>)}</ul></div>}
  </section>
}
