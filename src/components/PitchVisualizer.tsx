...
import { midiToNoteName } from '../music/noteUtils'
import { trackDuration } from '../music/referenceTrack'

interface Props { track: ReferenceTrack; frames: EvaluatedFrame[]; elapsed: number; running: boolean }

const ZOOM_LEVELS = [80, 140, 220]

export function PitchVisualizer({ track, frames, elapsed, running }: Props) {
    ...
    <div className="visualizer-wrap" ref={scrollRef}>
      <svg className="visualizer" width={width} height={height} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Timeline de pitch">
        {Array.from({ length: maxMidi - minMidi + 1 }, (_, index) => maxMidi - index).map((midi) => <g key={midi}>
          ...
          <text x={left - 8} y={y(midi) + 4} textAnchor="end">{midiToNoteName(midi)}</text>
        </g>)}
        ...
        {track.notes.map((note) => <g key={note.id}>
          ...
          <text className="note-label" x={x(note.start) + 5} y={y(note.midi) - 12}>{note.pitch}</text>
        </g>)}
