...
export function MusicViewControls({ view, naming, clef, disabled, onViewChange, onNamingChange, onClefChange, onAddNote }: Props) {
  return <div className="music-view-controls">
    ...
    <fieldset disabled={disabled}><legend>Rótulos das notas</legend><button className={naming === 'letter' ? 'selected' : ''} onClick={() => onNamingChange('letter')}>C, D, E</button><button className={naming === 'solfege' ? 'selected' : ''} onClick={() => onNamingChange('solfege')}>Dó, Ré, Mi</button><button className={naming === 'hidden' ? 'selected' : ''} onClick={() => onNamingChange('hidden')}>Ocultar</button></fieldset>
