...
export function CurrentNote({ expected, detected, differenceCents, volume, naming }: Props) {
  ...
  return <section className="current-card" aria-live="polite">
    <div><span>Nota esperada</span><strong>{expected ? midiToDisplayName(expected.midi, feedbackNaming) : '—'}</strong></div>
    <div><span>Nota detectada</span><strong>{detected ? midiToDisplayName(Math.round(detected.midi), feedbackNaming) : '—'}</strong></div>
