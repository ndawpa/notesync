...
import type { ReferenceNote } from '../types/music'
import { pitchStatus } from '../scoring/pitchScoring'

interface Props { expected?: ReferenceNote; detected?: PitchFrame; differenceCents?: number; volume: number }

export function CurrentNote({ expected, detected, differenceCents, volume }: Props) {
  ...
  return <section className="current-card" aria-live="polite">
    <div><span>Nota esperada</span><strong>{expected?.pitch || '—'}</strong></div>
    <div><span>Nota detectada</span><strong>{detected?.note || '—'}</strong></div>
